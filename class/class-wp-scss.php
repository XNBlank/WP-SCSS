...
class Wp_Scss
{
    ...
    public function needs_compiling()
    {
        ...
        $scss_outdated = false;
        $saved_checksums = get_option('wpscss_checksums');
        if(empty($saved_checksums) || !is_array($saved_checksums)) {
            // Option doesn't exist. We should make it.
            $saved_checksums = array();
        }
        ...
        foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($this->scss_dir), FilesystemIterator::SKIP_DOTS) as $sfile) {
            if (pathinfo($sfile->getFilename(), PATHINFO_EXTENSION) == 'scss') {
                ...
                $file_checksum = md5_file($sfile->getPath().'/'.$filename);
                $checked = false;

                if(!empty($saved_checksums)) {
                    foreach ($saved_checksums as $checksum) {
                        if (!empty($checksum)) {
                            // File checksum exists, compare with current checksum
                            if ($checksum == $file_checksum) {
                                // It matches, so we can skip to the next file.
                                $checked = true;
                                continue 2;
                            }
                        }
                    ...
                    if(!$checked) {
                        // File is outdated. Needs to be recompiled.
                        $saved_checksums[sanitize_text_field($filename)] = $file_checksum;
                        $scss_outdated = true;
                        $checked = true;
                        continue;
                    }
                ...
                if (!$checked) {
                    // File has never been checksummed
                    $scss_outdated = true;
                    // Update the checksum
                    $saved_checksums[sanitize_text_field($filename)] = $file_checksum;
                }
        ...
        if ($scss_outdated) {
            // Update the database
            update_option('wpscss_checksums', $saved_checksums);
        }

        return $scss_outdated;
